...
def branch_if_char_not_recruited(char: int, dest: str):
    return [
        asm.LDA(char_recruited_event_byte(char), asm.ABS),
        asm.BIT(char_recruited_event_bit(char), asm.IMM8),
        asm.BEQ(dest),
...
def branch_if_char_available(char: int, dest: str):
    return [
...
def branch_if_char_not_available(char: int, dest: str):
    return [
        asm.LDA(char_available_event_byte(char), asm.ABS),
        asm.BIT(char_available_event_bit(char), asm.IMM8),
        asm.BEQ(dest),
...
def branch_if_event_bit_set(event: int, dest: str):
    return [
...
def branch_if_event_bit_clear(event: int, dest: str):
    return [
        asm.LDA(ram_event_byte(event), asm.ABS),
        asm.BIT(ram_event_bit(event), asm.IMM8),
        asm.BEQ(dest),
