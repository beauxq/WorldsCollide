...
add_vigor = add_stat_all(VIGOR_ADDRESS, "vigor")
sub_vigor = sub_stat_all(VIGOR_ADDRESS, "vigor")

class Field(field_result.Result):
    def src(self, count):
        if count < 0:
            return [
                field.LongCall(START_ADDRESS_SNES + sub_vigor, -count),
            ]
        elif count > 0:
            return [
                field.LongCall(START_ADDRESS_SNES + add_vigor, count),
            ]
        return []
    ...
    def src(self, count):
        if count < 0:
            return [
                ...
                asm.JSL(START_ADDRESS_SNES + sub_vigor),
            ]
        elif count > 0:
            return [
                asm.LDA(count, asm.IMM8),
                asm.STA(field.LongCall.ARG_ADDRESS, asm.DIR),
                asm.JSL(START_ADDRESS_SNES + add_vigor),
            ]
        return []
