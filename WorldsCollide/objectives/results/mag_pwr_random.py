...
def add_mag_pwr(character):
    if character not in character_add:
        character_add[character] = add_stat_character(character, MAG_PWR_ADDRESS, "mag_pwr")
    return character_add[character]

def sub_mag_pwr(character):
    if character not in character_sub:
        character_sub[character] = sub_stat_character(character, MAG_PWR_ADDRESS, "mag_pwr")
...
class Field(field_result.Result):
    def src(self, count, character_name, character):
        if count < 0:
            return [
                field.LongCall(START_ADDRESS_SNES + sub_mag_pwr(character), -count),
            ]
        elif count > 0:
            return [
                field.LongCall(START_ADDRESS_SNES + add_mag_pwr(character), count),
            ]
        return []
    ...
    def src(self, count, character_name, character):
        if count < 0:
            return [
                ...
                asm.JSL(START_ADDRESS_SNES + sub_mag_pwr(character)),
            ]
        elif count > 0:
            return [
                asm.LDA(count, asm.IMM8),
                asm.STA(field.LongCall.ARG_ADDRESS, asm.DIR),
                asm.JSL(START_ADDRESS_SNES + add_mag_pwr(character)),
            ]
        return []
